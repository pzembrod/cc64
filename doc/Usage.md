...
- _nnn_ `set-himem`
  - sets the upper limit of the compiler's memory to _nnn_. On C64 and C16
    the default value is chosen to leave space for [`c-charset`](#c-charset)
    which cc64 recognizes and activates if it is installed. If a c-chargen
    rom generated by [c-char-rom-gen](#c-char-rom-gen) is used, e.g. in an
    emulator, or if no C charset (which provides the characters \^_{|}~) is
    desired, then himem can be set higher.
    -  On the C64 the default himem is $cbd0. [`c-charset`](#c-charset) uses
       $cbd0-$cfff. Without c-charset himem can be set to $dooo.
    -  On the C16 the default himem is $f000. [`c-charset`](#c-charset) uses
       $f000-$f812. Without c-charset himem can be set to $fd00.
    -  On the X16 the default himem is $9f00 and cannot be set higher.
       [`c-charset`](#c-charset) uses $0400-$04df, so it doesn't compete
       with cc64 for main memory.
- _nnn_ `set-heap`
...
- _nnn_ `set-symtab%`
  - sets the percentage of free memory that should be used for the symbol
    table. This includes both local and global symbols and also the hash
    table for global symbols. This should be increased if a symbol table
    overflow error occurs. It should be decreased if a `function too long`
    errir occurs and thus more memory is needed for code. Code and symbol
    table share the free available memory.
  - Not available on the X16 where a fixed code buffer of 8 kB lives in
banked RAM and all free memory is used for the symbol table.
- _nnn_ `set-hash%`
  - sets the percentage of symbol table memory that should be used for the
    hash table for global symbols. Could be increased if the stats after
    compiling show a high hash collision rate. Could be decreased if memory
    is tight and code with both many symbols and large functions are to be
    compiled.
